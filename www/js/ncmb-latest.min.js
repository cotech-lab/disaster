...
(function()
{
    ...
    M.size = function(N)
    {
        ...
        return (N.length === +N.length) ? N.length : M.keys(N)
            .length
    };
    ...
    M.clone = function(N)
    {
        ...
        return M.isArray(N) ? N.slice() : M.extend(
        {}, N)
    };
    M.tap = function(O, N)
    {
    ...
    {
        return isFinite(N) && !isNaN(NCMBFloat(N))
    };
    M.isNaN = function(N)
    {
    ...
    var K = {
        escape: new RegExp("[" + M.keys(n.escape)
            .join("") + "]", "g"),
        unescape: new RegExp("(" + M.keys(n.unescape)
            .join("|") + ")", "g")
    };
    ...
    M.template = function(V, Q, P)
    {
        var O;
        P = M.defaults(
        {}, P, M.templateSettings);
        var R = new RegExp([(P.escape || w)
            .source, (P.interpolate || w)
            .source, (P.evaluate || w)
            .source
        ].join("|") + "|$", "g");
        var S = 0;
...
.call(this);
var CryptoJS = CryptoJS || function(t, s)
{
    var w = {},
        v = w.lib = {},
        n = v.Base = function()
        ...
        p = v.WordArray = n.extend(
        {
            init: function(b, e)
            {
                b = this.words = b || [];
                this.sigBytes = e != s ? e : 4 * b.length
            },
            toString: function(b)
            {
                return (b || j)
            ...
            concat: function(f)
            {
                ...
                if (k % 4)
                {
                    for (var e = 0; e < f; e++)
                    {
                        g[k + e >>> 2] |= (h[e >>> 2] >>> 24 - 8 * (e % 4) & 255) << 24 - 8 * ((k + e) % 4)
                    }
                }
                else
                {
        ...
        j = d.Hex = {
            ...
            {
                for (var f = g.words, g = g.sigBytes, k = [], l = 0; l < g; l++)
                {
                    var h = f[l >>> 2] >>> 24 - 8 * (l % 4) & 255;
                    k.push((h >>> 4)
                        .toString(16));
                    k.push((h & 15)
                        .toString(16))
        ...
        x = v.BufferedBlockAlgorithm = n.extend(
        {
            reset: function()
            {
                this._data = p.create();
            ...
            _process: function(k)
            {
                var h = this._data,
                    ...
                    o = this.blockSize,
                    l = r / (4 * o),
                    l = k ? t.ceil(l) : t.max((l | 0) - this._minBufferSize, 0),
                    k = l * o,
                    r = t.min(4 * k, r);
                if (k)
                {
...
(function(g)
{
    ...
    (function()
    {
        ...
        for (var h = 2, k = 0; 64 > k;)
        {
            m(h) && (8 > k && (c[k] = l(g.pow(h, 0.5))), p[k] = l(g.pow(h, 1 / 3)), k++), h++
        }
    })();
    var a = [],
        b = b.SHA256 = n.extend(
        {
            _doReset: function()
            {
                this._hash = j.create(c.slice(0))
            },
            _doProcessBlock: function(x, w)
            {
                for (var A = this._hash.words, v = A[0], B = A[1], s = A[2], y = A[3], z = A[4], u = A[5], q = A[6], r = A[7], o = 0; 64 > o; o++)
                {
                    if (16 > o)
                    {
                        a[o] = x[w + o] | 0
                    ...
                    else
                    {
                        var t = a[o - 15],
                            m = a[o - 2];
                        a[o] = ((t << 25 | t >>> 7) ^ (t << 14 | t >>> 18) ^ t >>> 3) + a[o - 7] + ((m << 15 | m >>> 17) ^ (m << 13 | m >>> 19) ^ m >>> 10) + a[o - 16]
                    }
                    t = r + ((z << 26 | z >>> 6) ^ (z << 21 | z >>> 11) ^ (z << 7 | z >>> 25)) + (z & u ^ ~z & q) + p[o] + a[o];
                    m = ((v << 30 | v >>> 2) ^ (v << 19 | v >>> 13) ^ (v << 10 | v >>> 22)) + (v & B ^ v & s ^ B & s);
                    r = q;
                    q = u;
                    u = z;
                    z = y + t | 0;
            ...
            _doFinalize: function()
            {
                ...
                l[k >>> 5] |= 128 << 24 - k % 32;
                l[(k + 64 >>> 9 << 4) + 15] = h;
                m.sigBytes = 4 * l.length;
                this._process()
            }
        });
    d.SHA256 = n._createHelper(b);
    d.HmacSHA256 = n._createHmacHelper(b)
})(Math);
(function()
{
    var b = CryptoJS,
    ...
    b.algo.HMAC = b.lib.Base.extend(
    {
        init: function(s, p)
        {
            s = this._hasher = s.create();
            "string" == typeof p && (p = a.parse(p));
            var n = s.blockSize,
                l = 4 * n;
            p.sigBytes > l && (p = s.finalize(p));
            for (var g = this._oKey = p.clone(), j = this._iKey = p.clone(), d = g.words, c = j.words, t = 0; t < n; t++)
            {
                d[t] ^= 1549556828, c[t] ^= 909522486
            }
            g.sigBytes = j.sigBytes = l;
            this.reset()
        },
        reset: function()
        {
        ...
        finalize: function(d)
        {
            var c = this._hasher,
                d = c.finalize(d);
            c.reset();
            return c.finalize(this._oKey.clone()
                .concat(d))
        }
    })
})();
    ...
    var c = d.Base64 = {
        ...
        parse: function(o)
        {
            ...
            for (var k = 0; k < m; k++)
            {
                if (k % 4)
                {
                    var j = g.indexOf(o.charAt(k - 1)) << ((k % 4) * 2);
...
(function(a)
{
    ...
    b.serverURL = "https://mb.api.cloud.nifty.com/2013-09-01/";
    b.initialize = function(e, f)
    {
        b._initialize(e, f)
    };
    b._initialize = function(e, f)
    {
        b.applicationKey = e;
        b.clientKey = f
    };
    b.enableResponseValidation = function(e)
    {
        b.responseValidationFlag = e
    };
    b.responseValidationFlag = false;
    b._getNCMBPath = function(e)
    {
        if (!b.applicationKey)
        {
        ...
        return "NCMB/" + b.applicationKey + "/" + e
    };
    b._NCMBDate = function(e)
    {
        var m = new RegExp("^([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,2})T([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})(.([0-9]+))?Z$");
        var k = m.exec(e);
        if (!k)
        {
            return null
        ...
        var f = k[6] || 0;
        var j = k[8] || 0;
        return new Date(Date.UTC(n, l, o, h, g, f, j))
    };
    b._createSignature = function(G, m, e, s, o, n, F)
    {
        var B = "";
        var j = b.applicationKey;
        var x = n;
        var w = b.clientKey;
        var y = o;
        var v = encodeURI(s);
        var g = v.substring(v.lastIndexOf("//") + 2);
        ...
        if (p == -1)
        {
            r = v.substring(v.lastIndexOf(f) + f.length)
        }
        else
        {
            var l = v.substring(p + 1);
            r = v.substring(v.lastIndexOf(f) + f.length, p);
            g = l.split("&");
            for (var D = 0; D < g.length; D++)
            {
                p = g[D].indexOf("=");
                A[g[D].substring(0, p)] = g[D].substring(p + 1)
            }
        }
        A.SignatureMethod = "HmacSHA256";
        A.SignatureVersion = "2";
        A["X-NCMB-Application-Key"] = j;
        A["X-NCMB-Timestamp"] = x;
        var u = {};
        var z = [];
        var C, D, E;
        for (C in A)
        ...
        var h = CryptoJS.HmacSHA256(t, w);
        var B = CryptoJS.enc.Base64.stringify(h);
        return B
    };
    b._checkResponseSignature = function(g, n, l, m, f, e, k, h)
    {
        if (JSON.stringify(h) != "{}")
        {
            var j = b._createSignature(n, l, m, f, e, k, unescape(JSON.stringify(h)))
        }
        else
        {
            var j = b._createSignature(n, l, m, f, e, k, null)
        }
        if ((j != null) && (j != g))
        {
            return false
    ...
    b._ajax = function(q, o, e, g, l, h, n, r, p)
    {
        var u = {
            success: r,
            error: p
            ...
            if (s.readyState === 4)
            {
                ...
                if (s.status >= 200 && s.status < 300)
                {
                    ...
                    try
                    {
                        if (s.responseText)
                        {
                            if (q != "files" || e != "GET" || o == null)
                            {
                                v = JSON.parse(s.responseText)
                            }
                            else
                            {
                                v = s.responseText
        ...
        s.setRequestHeader("X-NCMB-Application-Key", b.applicationKey);
        s.setRequestHeader("X-NCMB-Timestamp", n);
        s.setRequestHeader("X-NCMB-Signature", h);
        var k = b.User.current();
        if (k)
        {
            var f = k._sessionToken;
            s.setRequestHeader("X-NCMB-Apps-Session-Token", f)
        }
        if (q == "files" && e == "POST")
        {
            var j = new FormData();
            j.append("file", l.file);
            j.append("acl", l.acl);
            s.send(j)
        }
        else
        ...
        return t._thenRunCallbacks(u)
    };
    b._extend = function(e, f)
    {
        var g = d(this, e, f);
        g.extend = this.extend;
        return g
    };
    b._request = function(o, l, m, e, h)
    {
        if (!b.applicationKey)
        {
        ...
        if (l == "installation" && o == "classes")
        {
            o = "installations";
            l = null
        }
        if (o !== "classes" && o !== "batch" && o !== "users" && o !== "files" && o !== "requestPasswordReset" && o !== "requestMailAddressUserEntry" && o !== "mailAddressConfirm" && o !== "login" && o !== "logout" && o !== "push" && o !== "roles" && o !== "installations")
        {
            throw "Bad route: '" + o + "'."
        }
        var f = b.serverURL;
        ...
        if (h)
        {
            if (h.where || h.include || h.order || h.skip || h.count || h.limit)
            {
                f += "?"
            }
            if (o == "login")
            {
                if (h.userName || h.password || h.mailAddress)
                {
                    f += "?"
                }
            }
            if (h.where)
            {
                if (f.charAt(f.length - 1) == "?")
                {
                    f += "where=" + JSON.stringify(h.where)
                }
                else
                {
                    f += "&where=" + JSON.stringify(h.where)
            ...
            if (h.count)
            {
                if (f.charAt(f.length - 1) == "?")
                {
                    f += "count=" + JSON.stringify(h.count)
                }
                else
                {
                    f += "&count=" + JSON.stringify(h.count)
            ...
            if (h.limit)
            {
                if (f.charAt(f.length - 1) == "?")
                {
                    f += "limit=" + JSON.stringify(h.limit)
                }
                else
                {
                    f += "&limit=" + JSON.stringify(h.limit)
            ...
            if (o == "login")
            {
                ...
                if (h.mailAddress)
                {
                    if (f.charAt(f.length - 1) == "?")
                    {
                        f += "mailAddress=" + h.mailAddress
                    }
                    else
                    {
                        f += "&mailAddress=" + h.mailAddress
                    }
                    delete h.mailAddress
                }
            }
        }
        var g = new Date();
        if (!Date.prototype.toISOString)
        {
            (function()
            {
                ...
                {
                    return this.getUTCFullYear() + "-" + p(this.getUTCMonth() + 1) + "-" + p(this.getUTCDate()) + "T" + p(this.getUTCHours()) + ":" + p(this.getUTCMinutes()) + ":" + p(this.getUTCSeconds()) + "." + String((this.getUTCMilliseconds() / 1000)
                            .toFixed(3))
                        .slice(2, 5) + "Z"
                }
            }())
        ...
        var k = n;
        signature = b._createSignature(o, l, m, f, e, k, null);
        var j = h;
        return b._ajax(o, l, e, f, j, signature, k)
            .then(function(q, p, s)
            {
                if (b.responseValidationFlag)
                {
                    var t = s.getResponseHeader("X-NCMB-Response-Signature");
                    if (typeof(t) != "undefined" && t != null && b.enableResponseValidation)
                    {
                        if (b._checkResponseSignature(t, o, l, m, f, e, k, q))
                        {}
                        else
                        {
                            var r = new b.Error("E100001", "Authentication error by response signature incorrect.");
                            return b.Promise.error(r)
                        }
                    }
                }
                return b.Promise.as(q, p, s)
            }, function(q, p)
            {
                var s;
                if (q && q.responseText)
                {
                    try
                    {
                        ...
                        if (r)
                        {
                            s = new b.Error(r.code, r.error)
                        }
                    }
                    catch (u)
                    {}
                }
                s = s || new b.Error(-1, q.responseText);
                if (b.responseValidationFlag)
                {
                    if (!((s) && (s.code) && (s.code == "E404002") && (s.code == "E405001") && (s.code == "E415001")))
                    {
                        var v = q.getResponseHeader("X-NCMB-Response-Signature");
                        if (typeof(v) != "undefined" && v != null && b.enableResponseValidation)
                        {
                            if (b._checkResponseSignature(v, o, l, m, f, e, k, JSON.parse(q.responseText)))
                            {}
                            else
                            {
                                s.code = "E100001";
                                s.message = "Authentication error by response signature incorrect."
                            }
                        }
                    }
                }
                var t = b.User.current();
                if (s.code == "E401001")
                {
                    if (t._sessionToken != null)
                    {
                        b.User.logOut()
                    }
                }
                return b.Promise.error(s)
            })
    };
    b._getValue = function(e, f)
    {
        if (!(e && e[f]))
        {
            return null
        }
        return b._.isFunction(e[f]) ? e[f]() : e[f]
    };
    b._encode = function(j, h, g)
    {
        var f = b._;
        if (j instanceof b.Object)
        {
            if (g)
            {
                throw "NCMB.Objects not allowed here"
            }
            if (!h || f.include(h, j) || !j._hasData)
            {
                return j._toPointer()
            }
            if (!j.dirty())
            {
                h = h.concat(j);
                return b._encode(j._toFullJSON(h), h, g)
            }
            throw "Tried to save an object with a pointer to a new, unsaved object."
        }
        if (j instanceof b.ACL)
        {
            return j.toJSON()
        }
        ...
        if (f.isArray(j))
        {
            return f.map(j, function(k)
            {
                return b._encode(k, h, g)
            })
        }
        if (f.isRegExp(j))
        {
        ...
        if (j instanceof b.Relation)
        {
            return j.toJSON()
        }
        if (j instanceof b.Op)
        {
            return j.toJSON()
        }
    ...
    b._decode = function(h, l)
    {
        ...
        if (f.isArray(l))
        {
            b._arrayEach(l, function(o, n)
            {
                l[n] = b._decode(n, o)
            });
            return l
        }
        if (l instanceof b.Object)
        {
            return l
        }
        if (l instanceof b.File)
        {
            return l
        }
        if (l instanceof b.Op)
        {
            return l
        }
        if (l.__op)
        {
            return b.Op._decode(l)
        }
        if (l.__type === "Pointer")
        {
            var m = b.Object._create(l.className);
            m._finishFetch(
            {
                objectId: l.objectId
            }, false);
        ...
        if (l.__type === "Object")
        {
            ...
            delete l.__type;
            delete l.className;
            var e = b.Object._create(j);
            e._finishFetch(l, true);
            return e
        }
        if (l.__type === "Date")
        {
            return b._NCMBDate(l.iso)
        }
        if (l.__type === "GeoPoint")
        {
            return new b.GeoPoint(
            {
                latitude: l.latitude,
        ...
        {
            var k = new b.Relation(null, h);
            k.targetClassName = l.className;
            return k
        }
        if (l.__type === "File")
        {
            var g = new b.File(l.name);
            g._url = l.url;
            return g
        }
        b._objectEach(l, function(o, n)
        {
            l[n] = b._decode(n, o)
        });
        return l
    };
    b._arrayEach = b._.each;
    b._traverse = function(f, g, e)
    {
        if (f instanceof b.Object)
        {
            e = e || [];
            if (b._.indexOf(e, f) >= 0)
            {
                return
            }
            e.push(f);
            b._traverse(f.attributes, g, e);
            return g(f)
        }
        if (f instanceof b.Relation || f instanceof b.File)
        {
            return g(f)
        }
        if (b._.isArray(f))
        {
            b._.each(f, function(k, j)
            {
                var h = b._traverse(k, g, e);
                if (h)
                {
                    f[j] = h
                }
    ...
    b._objectEach = b._each = function(f, g)
    {
        var e = b._;
        if (e.isObject(f))
        {
            e.each(e.keys(f), function(h)
            {
                g(f[h], h)
            })
        }
    ...
    b._isNullOrUndefined = function(e)
    {
        return b._.isNull(e) || b._.isUndefined(e)
    };
    b._createUuid = function()
    {
        var e = function()
        {
...
(function(a)
{
    ...
    b.extend(c.Promise,
    {
        is: function(d)
        {
            if (d && d.then && b.isFunction(d.then))
            {
    ...
    b.extend(c.Promise.prototype,
    {
        ...
        then: function(d, f)
        {
            ...
            var h = function()
            {
                ...
                if (j.length === 1 && c.Promise.is(j[0]))
                {
                    j[0].then(function()
                    {
        ...
        trigger: function(h)
        {
            ...
            while (m)
            {
                ...
                if (l)
                {
                    ...
                    while ((l = l.next) !== f)
                    {
                        l.callback.apply(l.context || this, j)
                    }
                }
                l = k;
                if (l)
                {
                    ...
                    while ((l = l.next) !== f)
                    {
                        l.callback.apply(l.context || this, e)
                    }
                }
                m = h.shift()
            }
...
(function(a)
{
    ...
    {
        return c.Object._deepSaveAsync(e)
            ._thenRunCallbacks(d)
    };
    b.extend(c.Object.prototype, c.Events,
    {
        ...
        escape: function(e)
        {
            var d = this._escapedAttributes[e];
            if (d)
            {
                return d
            }
        ...
        _mergeMagicFields: function(e)
        {
            ...
            c._arrayEach(f, function(g)
            {
                if (e[g])
                {
                    ...
                    {
                        if ((g === "createdAt" || g === "updatedAt") && !b.isDate(e[g]))
                        {
                            d[g] = c._NCMBDate(e[g])
                        }
                        else
        ...
        _cancelSave: function()
        {
            ...
            c._objectEach(d, function(k, h)
            {
                var j = d[h];
                var g = f[h];
                if (j && g)
                {
                    f[h] = g._mergeWithPrevious(j)
                }
                else
                {
                    if (j)
        ...
        _finishSave: function(f)
        {
            ...
            var e = this;
            c._objectEach(f, function(j, h)
            {
                e._serverData[h] = c._decode(h, j);
                var k = c._traverse(e._serverData[h], function(l)
                {
                    if (l instanceof c.Object && d[l.id])
                    {
                        return d[l.id]
                    }
                });
        ...
        {
            var k, g;
            if (b.isObject(j) || c._isNullOrUndefined(j))
            {
                k = j;
                c._objectEach(k, function(o, n)
                {
                    k[n] = c._decode(n, o)
                });
                m = h
            ...
            if (k instanceof c.Object)
            {
                k = k.attributes
            }
            if (m.unset)
            {
                c._objectEach(k, function(n, o)
                {
                    k[o] = new c.Op.Unset()
                })
            }
            var f = b.clone(k);
            var l = this;
            c._objectEach(f, function(n, o)
            {
                if (n instanceof c.Op)
                {
                    f[o] = n._estimate(l.attributes[o], l, o);
                    if (f[o] === c.Op._UNSET)
                    {
                        delete f[o]
                    }
            ...
            if (!this._validate(k, m))
            {
                return false
            }
            this._mergeMagicFields(k);
            m.changes = {};
            var d = this._escapedAttributes;
            var e = this._previousAttributes ||
            {};
            c._arrayEach(b.keys(k), function(o)
            {
                var q = k[o];
                if (q instanceof c.Relation)
                {
                    q.parent = l
                }
                if (!(q instanceof c.Op))
                {
                    q = new c.Op.Set(q)
                }
                var n = true;
                if (q instanceof c.Op.Set && b.isEqual(l.attributes[o], q.value))
                {
                    n = false
                }
                if (n)
                ...
                var p = b.last(l._opSetQueue);
                p[o] = q._mergeWithPrevious(p[o]);
                l._rebuildEstimatedDataForKey(o);
                if (n)
                {
                    l.changed[o] = l.attributes[o];
                    if (!m.silent)
                    {
                        l._pending[o] = true
                    }
        ...
        unset: function(e, d)
        {
            d = d ||
            {};
            return this.set(e, null, d)
        },
        increment: function(e, d)
        {
            if (b.isUndefined(d) || b.isNull(d))
            {
                d = 1
            }
            return this.set(e, new c.Op.Increment(d))
        },
        add: function(d, e)
        {
        ...
        fetch: function(e)
        {
            ...
            return f.then(function(h, g, j)
                {
                    d._finishFetch(d.parse(h, g, j), true);
                    return d
                })
        ...
        save: function(o, q)
        {
            ...
            if (!q && m)
            {
                var n = b.reject(m, function(s, r)
                {
                    return b.include(["success", "error", "wait"], r)
                });
                if (n.length === 0)
                {
                    var p = true;
                    if (b.has(m, "success") && !b.isFunction(m.success))
                    {
                        p = false
                    }
                    if (b.has(m, "error") && !b.isFunction(m.error))
                    {
                        p = false
                    }
                    if (p)
                    {
                        return this.save(null, m)
                    }
                }
            }
            q = b.clone(q) ||
            ...
            if (m && !this.set(m, j))
            {
                return c.Promise.error(e)
                    ._thenRunCallbacks(q, this)
            }
            var g = this;
            g._refreshCache();
            var k = [];
            var d = [];
            c.Object._findUnsavedChildren(g.attributes, k, d);
            if (k.length + d.length > 0)
            {
                return c.Object._deepSaveAsync(this.attributes)
                    .then(function()
                    {
                        return g.save(null, q)
                    }, function(r)
                    {
                        return c.Promise.error(r)
                            ._thenRunCallbacks(q, g)
                    })
            }
            this._startSave();
            ...
            this._allPreviousSaves = this._allPreviousSaves._continueWith(function()
            {
                ...
                var s = g._getSaveJSON();
                var r = "classes";
                var t = g.className;
                var u = c._request(r, t, g.id, v, s);
                u = u.then(function(z, w, y)
                    {
                        var x = g.parse(z, w, y);
                        if (q.wait)
                        {
        ...
        destroy: function(f)
        {
            ...
            var g = c._request("classes", this.className, this.id, "DELETE");
        ...
        change: function(f)
        {
            ...
            this._changing = true;
            var e = this;
            c._objectEach(this._silent, function(j)
            {
                e._pending[j] = true
            });
            var g = b.extend(
            {}, f.changes, this._silent);
            this._silent = {};
            c._objectEach(g, function(k, j)
            {
                e.trigger("change:" + j, e, e.get(j), f)
            });
            if (h)
            {
                return this
        ...
        validate: function(e, d)
        {
            ...
            {
                return new c.Error(c.Error.OTHER_CAUSE, "ACL must be a NCMB.ACL.")
            }
            return false
        },
    ...
    c.Object.extend = function(g, f, h)
    {
        if (!b.isString(g))
        {
            ...
            {
                throw new Error("NCMB.Object.extend's first argument should be the className.")
            }
        }
        if (g === "User")
        {
        ...
        d.extend = function(k)
        {
            if (b.isString(k) || (k && b.has(k, "className")))
            {
                return c.Object.extend.apply(d, arguments)
            }
            var j = [g].concat(c._.toArray(arguments));
            return c.Object.extend.apply(d, j)
        };
        ...
        return j.then(function()
            {
                ...
                }, function()
                {
                    ...
                    var l = c.Promise.when(b.map(n, function(o)
                    {
                        return o._allPreviousSaves || c.Promise.as()
                    }));
                    var m = new c.Promise();
                    c._arrayEach(n, function(o)
                    {
                        o._allPreviousSaves = m
                    });
                    return l._continueWith(function()
                    {
                        return c._request("batch", null, null, "POST",
                            {
                                requests: b.map(n, function(o)
                                {
                                    var p = o._getSaveJSON();
                                    var r = "POST";
                                    var q = "/2013-09-01/classes/" + o.className;
                                    if (o.id)
                                    {
                                        q = q + "/" + o.id;
                                        r = "PUT"
                            ...
                            {
                                var q;
                                c._arrayEach(n, function(s, t)
                                {
                                    if (p[t].success)
                                    {
                                        s._finishSave(s.parse(p[t].success, o, r))
                                    }
                                    else
                                    {
                                        q = q || p[t].error;
                                ...
                                if (q)
                                {
                                    return c.Promise.error(new c.Error(q.code, q.error))
                                }
                            })
                            .then(function(o)
                            {
...
(function(a)
{
    ...
    c.Op.Add = c.Op._extend(
    {
        ...
        toJSON: function()
        {
            return {
                __op: "Add",
                objects: c._encode(this.objects())
            }
        },
        _mergeWithPrevious: function(d)
        {
        ...
        _estimate: function(d)
        {
            ...
            else
            {
                ...
                c._arrayEach(this.objects(), function(h)
                {
                    ...
                    {
                        var g = b.find(e, function(j)
                        {
                            return (j instanceof c.Object) && (j.id === h.id)
                        });
                        if (!g)
                        {
                            e.push(h)
...
(function(b)
{
    ...
    var c = function(h)
    {
        if (h < 26)
        {
            return String.fromCharCode(65 + h)
        }
        if (h < 52)
        {
        ...
        if (h < 62)
        {
            return String.fromCharCode(48 + (h - 52))
        }
        if (h === 62)
        {
            return "+"
        ...
        throw "Tried to encode large digit " + h + " in base64."
    };
    var a = function(j)
    {
        var h = [];
        h.length = Math.ceil(j.length / 3);
        d.times(h.length, function(n)
        {
            var m = j[n * 3];
            var l = j[n * 3 + 1] || 0;
            var k = j[n * 3 + 2] || 0;
            var p = (n * 3 + 1) < j.length;
            var o = (n * 3 + 2) < j.length;
            h[n] = [c((m >> 2) & 63), c(((m << 4) & 48) | ((l >> 4) & 15)), p ? c(((l << 2) & 60) | ((k >> 6) & 3)) : "=", o ? c(k & 63) : "="].join("")
        });
        return h.join("")
    };
    var g = {
    ...
    var f = function(j, k)
    {
        ...
        h.onloadend = function()
        {
            if (h.readyState !== 2)
            {
                l.reject(new e.Error(-1, "Error reading file."));
                return
            }
            var n = h.result;
            var q = /\.([^.]*)$/.exec(j.name);
            ...
            var m = /^data:([^;]*);base64,(.*)$/.exec(n);
            var o = /^data:base64,(.*)$/.exec(n);
            if (!m)
            {
                if (!o)
                {
                    l.reject(new e.Error(-1, "Unable to interpret data URL: " + n));
                    return
                }
                else
                {
    ...
    e.File = e.Object.extend("File",
    {
        constructor: function(h, k, j, l)
        {
            if (window.cordova)
            {
                delete File
            ...
            this._type = j;
            this._newFlag = true;
            if (d.isString(h) && (l instanceof e.ACL))
            {
            ...
            {
                e.Object.prototype.constructor.call(this, h, k, j, l)
            }
            var n = /\.([^.]*)$/.exec(h);
            if (n)
            {
            ...
            if (d.isArray(k))
            {
                this._source = e.Promise.as(a(k), m)
            }
            else
            {
                if (k && k.base64)
                {
                    this._source = e.Promise.as(k.base64, m)
                }
                else
                {
                    if (typeof(File) !== "undefined" && k instanceof File)
                    {
                        this._source = f(k, j)
                    }
                    else
        ...
        save: function(j)
        {
            ...
            if (!h._previousSave)
            {
                h._previousSave = h._source.then(function(k, l)
                    {
                        h._dataBase64 = k;
                        var m = {};
                        m.file = h._data;
                        if (h.ACL)
                        {
                            m.acl = JSON.stringify(h.ACL)
                        }
                        else
                        {
                            m.acl = JSON.stringify(
                            {
                                "*":
                                {
                                    read: true,
                        ...
                        if (m.file)
                        {
                            return e._request("files", h._name, null, "POST", m)
                        }
                        else
                        {
                            return e._request("files", h._name, null, "PUT", m)
                        }
                    })
                    .then(function(k)
                    {
            ...
            return h._previousSave._thenRunCallbacks(j)
        },
        destroy: function(j)
        {
            j = j ||
            ...
            var l = function()
            {
                h.trigger("destroy", h, h.collection, j)
            };
            var k = e._request("files", self._name, null, "DELETE");
            return k.then(function()
                ...
                ._thenRunCallbacks(j, this)
        },
        fetch: function(j)
        {
            var h = this;
            return e._request("files", this._name, null, "GET", null)
                .then(function(k)
                {
                    var l = "";
                    for (i = 0; i < k.length; i++)
                    {
                        l += String.fromCharCode(k.charCodeAt(i) & 255)
                    }
                    h._data = l;
                    h._dataBase64 = btoa(unescape(encodeURIComponent(k)));
                    return h
                })
                ._thenRunCallbacks(j)
        },
        fetchImgSource: function(j)
        {
            var k = this.getData();
            if (k)
            {
                this._fetchImgSource(j, k)
            }
            else
            {
                var h = this;
                h.fetch()
                    .then(function()
                    {
                        h._fetchImgSource(j, h._data)
                    })
            }
        },
        _fetchImgSource: function(j, m)
        {
            var l = m.toString()
                .substring(0, 9);
            var h;
            ...
            else
            {
                ...
                {
                    if (l.match(/^GIF87a/) || l.match(/^GIF89a/))
                    {
                        h = "gif"
                    }
                    else
            ...
            var k = "data:image/" + h + ";base64," + window.btoa(m);
            j.src = k;
            return
        },
...
(function(a)
{
    ...
    c.Role = c.Object.extend("role",
    {
        ...
        {
            return this.set("roleName", e, d)
        },
        _setRoles: function(e, d)
        {
            return this.set("belongRole", e, d)
        },
        _setUsers: function(e, d)
        {
            return this.set("belongUser", e, d)
        },
        getUsers: function()
        {
            var d = this.relation("belongUser");
            d.targetClassName = "user";
            return d
        },
        getRoles: function()
        {
            var d = this.relation("belongRole");
            d.targetClassName = "role";
            return d
        },
        validate: function(f, e)
        {
            if ("roleName" in f && f.roleName !== this.getName())
            {
                var d = f.roleName;
...
(function(a)
{
    ...
    c.Query.prototype = {
        ...
        {
            var d = this;
            var f = c._request("classes", this.className, null, "GET", this.toJSON());
            return f.then(function(g)
                {
                    return b.map(g.results, function(h)
                    {
                        ...
                        if (g.className)
                        {
                            j = new c.Object(g.className)
                        }
                        else
                        {
                            j = new d.objectClass()
                        }
                        j._finishFetch(h, true);
                        return j
                    })
                })
                ._thenRunCallbacks(e)
        ...
        count: function(d)
        {
            ...
            f.limit = 0;
            f.count = 1;
            var e = c._request("classes", this.className, null, "GET", f);
        ...
        first: function(e)
        {
            ...
            var g = this.toJSON();
            g.limit = 1;
            var f = c._request("classes", this.className, null, "GET", g);
            return f.then(function(h)
                {
                    return b.map(h.results, function(j)
                    {
                        var k = new d.objectClass();
                        k._finishFetch(j, true);
                        return k
                    })[0]
                })
                ._thenRunCallbacks(e)
        ...
        collection: function(d, e)
        {
            e = e ||
            {};
            return new c.Collection(d, b.extend(e,
            {
        ...
        matchesKeyInQuery: function(f, e, h)
        {
            var g = h.toJSON();
            var d = {};
            d.className = h.className;
            d.where = g.where;
            this._addCondition(f, "$select",
            {
                query: d,
                key: e
            });
        ...
        relatedTo: function(e, g, f)
        {
            ...
            h.className = g;
            h.objectId = e;
            d._addCondition("$relatedTo", "object", h);
            d._addCondition("$relatedTo", "key", f);
            return this
        },
        select: function()
        {
                ...
                }, function()
                {
                    return f.find()
                        .then(function(k)
                        {
                            ...
                            {
                                if (k.length >= f._limit)
                                {
                                    f.greaterThan("objectId", k[k.length - 1].id)
                                }
                                else
                                {
                                    h = true
        ...
        {
            this.near(e, d);
            this._addCondition(e, "$maxDistanceInRadians", f);
            return this
        },
        withinMiles: function(e, d, f)
        {
            this.near(e, d);
            this._addCondition(e, "$maxDistanceInMiles", f);
            return this
        },
        withinKilometers: function(e, d, f)
        {
            this.near(e, d);
            this._addCondition(e, "$maxDistanceInKilometers", f);
            return this
        },
...
(function(a)
{
    ...
    var c = a.NCMB;
    var b = c._;
    c.User = c.Object.extend("user",
    {
        _isCurrentUser: false,
        _mergeMagicFields: function(d)
        {
            if (d.sessionToken)
            {
        ...
        _linkWith: function(k, f)
        {
            var e;
            if (b.isString(k))
            {
                e = k;
                k = c.User._authProviders[k]
            }
            else
            {
                e = k.getAuthType()
            ...
            if (b.has(f, "authData"))
            {
                ...
                if (e == "facebook")
                {
                    if (g[e])
                    {
                        if (g[e]["expiration_date"])
                        {
                            g[e]["expiration_date"] = {
                                __type: "Date",
                                iso: g[e]["expiration_date"]
                            }
                        }
                    }
                }
                ...
                var j = b.clone(f) ||
                {};
                j.success = function(l)
                {
                    l._handleSaveResult(true);
                    if (f.success)
                    {
        ...
        signUp: function(g, f)
        {
            ...
            f = f ||
            {};
            var h = (g && g.userName) || this.get("userName");
            if (!h || (h === ""))
            {
            ...
            return this.save(g, j)
        },
        logIn: function(e)
        {
            var d = this;
            var f = c._request("login", null, null, "GET", this.toJSON());
            return f.then(function(k, g, j)
                {
                    var h = d.parse(k, g, j);
                    d._finishFetch(h);
        ...
        save: function(h, e)
        {
            ...
            {};
            var k = b.clone(e);
            k.success = function(l)
            {
                l._handleSaveResult(false);
                if (e.success)
                {
                    e.success.apply(this, arguments)
                }
            };
            return c.Object.prototype.save.call(this, d, k)
        },
        fetch: function(d)
        {
            var e = d ? b.clone(d) :
        ...
        {
            return this.set("mailAddress", e, d)
        },
        authenticated: function()
        {
            return !!this._sessionToken && (c.User.current() && c.User.current()
                .id === this.id)
        }
    },
    {
        ...
        signUp: function(h, g, f, e)
        {
            ...
            f.userName = h;
            f.password = g;
            var d = c.Object._create("user");
            return d.signUp(f, e)
        },
        loginWithMailAddress: function(g, f, e)
        {
            var d = c.Object._create("user");
            d._finishFetch(
            {
        ...
        logOut: function()
        {
            ...
            c.localStorage.removeItem(c._getNCMBPath(c.User._CURRENT_USER_KEY))
        },
        ...
        requestAuthenticationMail: function(e, d)
        {
            var f = {
                mailAddress: e
            };
            var g = c._request("requestMailAddressUserEntry", null, null, "POST", f);
            return g._thenRunCallbacks(d)
        },
        current: function()
        {
            ...
            c.User._currentUserMatchesDisk = true;
            var d = c.localStorage.getItem(c._getNCMBPath(c.User._CURRENT_USER_KEY));
            if (!d)
            {
                return null
            }
            c.User._currentUser = c.Object._create("user");
        ...
        _saveCurrentUser: function(d)
        {
            ...
            c.localStorage.setItem(c._getNCMBPath(c.User._CURRENT_USER_KEY), JSON.stringify(e))
        },
        _registerAuthenticationProvider: function(d)
        {
            ...
            {
                c.User.current()
                    ._synchronizeAuthData(d.getAuthType())
            }
        },
        _logInWith: function(g, e)
        {
            if (g == "anonymous")
            {
                var f = {};
...
(function(a)
{
    ...
    c.AnonymousUtils = {
        isLink: function(d)
        {
            return d._isLinked("anonymous")
        },
        logIn: function(d)
        {
            return c.User._logInWith("anonymous", d)
        }
    }
}(this));
(function(a)
...
(function(a)
{
    ...
    {};
    var b = a.NCMB;
    b.Installation = b.Object.extend("installation");
    b.Push = b.Object.extend("push");
    b.Push.constructor = function(c)
    {
        this._data = c
    };
    b.Push.save = function(e, c)
    {
        if (!e)
        {
            if (this._data)
        ...
        if (e.searchCondition)
        {
            e.searchCondition = b._encode(e.searchCondition)
        }
        if (e.deliveryExpirationDate)
        {
            e.deliveryExpirationDate = b._encode(e.deliveryExpirationDate)
        }
        if (e.deliveryTime)
        {
            e.deliveryTime = b._encode(e.deliveryTime)
        }
        if (e.deliveryExpirationTime)
        {
            e.deliveryExpirationTime = b._encode(e.deliveryExpirationTime)
        }
        if (e.deliveryExpirationDate && e.deliveryExpirationTime)
        {
...
(function(a)
{
    ...
    var f = {
            ...
            FB.login(function(l)
            {
                if (l.authResponse)
                {
                    if (k.success)
                    {
                        k.success(j,
                        {
                            id: l.authResponse.userID,
                            access_token: l.authResponse.accessToken,
                            expiration_date: new Date(l.authResponse.expiresIn * 1000 + (new Date())
                                    .getTime())
                                .toJSON()
                        })
                    }
        ...
        restoreAuthentication: function(k)
        {
            if (k)
            {
                var j = {
                    ...
                    expiresIn: (d._NCMBDate(k.expiration_date.iso)
                        .getTime() - (new Date())
                        .getTime()) / 1000
                };
                var l = c.clone(e);
    ...
    d.FacebookUtils = {
        init: function(j)
        {
            ...
            e = c.clone(j) ||
            {};
            if (e.status && typeof(console) !== "undefined")
            {
                var k = console.warn || console.log || function() {}
            }
            e.status = false;
            FB.init(e);
            d.User._registerAuthenticationProvider(f);
            g = true
        },
        isLinked: function(j)
        {
            return j._isLinked("facebook")
        },
        logIn: function(k, j)
        {
            if (!k || c.isString(k))
            {
                ...
                h = k;
                return d.User._logInWith("facebook", j)
            }
            else
            {
                var l = c.clone(j) ||
                {};
                l.authData = k;
                return d.User._logInWith("facebook", l)
            }
        },
        link: function(j, l, k)
        {
        ...
        unlink: function(j, k)
        {
            ...
            return j._unlinkFrom("facebook", k)
        }
    }
}(this));
(function(a)
    ...
    d.extend(e.View.prototype, e.Events,
    {
        ...
        delegateEvents: function(g)
        {
            ...
            e._objectEach(g, function(m, l)
            {
                ...
                if (!m)
                {
                    throw new Error('Event "' + g[l] + '" does not exist')
                }
                var k = l.match(c);
                var j = k[1],
                    h = k[2];
                m = d.bind(m, f);
                j += ".delegateEvents" + f.cid;
                if (h === "")
                {
                    f.$el.bind(j, m)
                }
...
(function(b)
{
    ...
    c.extend(d.Router.prototype, d.Events,
    {
        ...
        route: function(g, h, j)
        {
            ...
            d.history.route(g, c.bind(function(l)
            {
                var k = this._extractParameters(g, l);
                if (j)
                {
                    j.apply(this, k)
                }
                this.trigger.apply(this, ["route:" + h].concat(k));
                d.history.trigger("route", this, h, k)
            }, this));
            return this
        },
...
(function(a)
{
    ...
    c.extend(d.History.prototype, d.Events,
    {
        ...
        start: function(h)
        {
            ...
            var k = (b.exec(navigator.userAgent.toLowerCase()) && (!f || f <= 7));
            if (k)
            {
            ...
            var j = l.pathname === this.options.root;
            ...
            else
            {
                if (this._wantsPushState && this._hasPushState && j && l.hash)
                {
                    ...
                    {}, document.title, l.protocol + "//" + l.host + this.options.root + this.fragment)
                }
            }
        ...
        loadUrl: function(h)
        {
            ...
            var f = c.any(this.handlers, function(j)
            {
                if (j.route.test(g))
                {
                    j.callback(g);
        ...
        navigate: function(h, g)
        {
            ...
            if (this._hasPushState)
            {
                if (j.indexOf(this.options.root) !== 0)
                {
                    j = this.options.root + j
                }
                this.fragment = j;
            ...
            else
            {
                ...
                {
                    this.fragment = j;
                    this._updateHash(window.location, j, g.replace);
                    if (this.iframe && (j !== this.getFragment(this.getHash(this.iframe))))
                    {
                        if (!g.replace)
                        {
                        ...
                        this._updateHash(this.iframe.location, j, g.replace)
                    }
                }
                else
                {
...
(function(a)
{
    ...
    var c = a.NCMB;
    var b = c._;
    c.GeoPoint = function(e, d)
    {
        if (b.isArray(e))
        {
            c.GeoPoint._validate(e[0], e[1]);
            this.latitude = e[0];
            this.longitude = e[1]
        }
        else
        {
            if (b.isObject(e))
            {
                c.GeoPoint._validate(e.latitude, e.longitude);
                this.latitude = e.latitude;
                this.longitude = e.longitude
            }
            else
            {
                if (b.isNumber(e) && b.isNumber(d))
                {
                    c.GeoPoint._validate(e, d);
                    this.latitude = e;
                    this.longitude = d
                }
                else
    ...
    c.GeoPoint._validate = function(e, d)
    {
        if (e < -90)
        {
            throw "NCMB.GeoPoint latitude " + e + " < -90.0."
        }
        if (e > 90)
        {
            throw "NCMB.GeoPoint latitude " + e + " > 90.0."
        ...
        if (d < -180)
        {
            throw "NCMB.GeoPoint longitude " + d + " < -180.0."
        }
        if (d > 180)
        {
            throw "NCMB.GeoPoint longitude " + d + " > 180.0."
